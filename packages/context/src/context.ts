...
export class Context {
  ...
  configure(key: string = '', env: string = ''): Binding {
    const keyForConfig = Binding.buildKeyForConfig(key, env);
  ...
  getConfigAsValueOrPromise<T>(
    ...
    const configKey = Binding.buildKeyWithPath(
      Binding.buildKeyForConfig(key, env),
      configPath,
    );
    ...
    while (true) {
      ...
      if (env) {
        ...
        keys.push(
          Binding.buildKeyWithPath(Binding.buildKeyForConfig(key), configPath),
        );
      }
